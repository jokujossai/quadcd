...
- The generator now honours `[Install]` sections of plain systemd unit files, creating `<target>.wants/<unit>` and `<target>.requires/<unit>` symlinks in the generator directory the same way Quadlet does for its own units. Previously a plain `.service`, `.timer`, `.socket` or other native unit synced by quadcd could never start at boot: generated units cannot be enabled with `systemctl enable`, and quadcd created no links, so nothing ever pulled the unit in. `WantedBy=` and `RequiredBy=` are read; the links are relative and resolve to the installed unit next to the `.wants`/`.requires` directory.
...
- **Breaking (behaviour):** Sync decides what to start from reverse dependencies instead of `systemctl is-enabled`. Every unit quadcd deploys is generated, so `is-enabled` reports `generated` for all of them and could not distinguish a unit boot would start from one it would not; sync could start units no reboot would, silently diverging running state from boot state. A changed unit is now restarted when it is active, and an inactive one is started only when some unit that wants or requires it is itself coming up — an approximation of what a reboot would bring up. Units an operator stopped by hand are no longer resurrected by a sync that touches their file. The refinements below (`BoundBy=`/`UpheldBy=`, "coming up" states, template instances, pre-pull) all build on this rule.
